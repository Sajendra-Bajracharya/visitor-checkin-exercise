...
I’d consider **deactivation enforcement** (Defects 7 and 8) the highest-risk area I found.
...
I'm asking because right now there's no admin screen anywhere in the app for
this the only way I could trigger it during testing was hitting the API
directly. If that's intentional for this release, Defects 7 and 8 still need
fixing regardless of how deactivation gets triggered. But if an admin UI was
actually expected to be part of this feature, that's a separate missing-piece
finding on top of the two defects, and it probably changes what "done" looks
like for sign-off.
